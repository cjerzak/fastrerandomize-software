...
#' 1. Verifies R-squared > InSampleR_threshold for Y0 and Y1 regressed on X
#' 2. Checks out-of-sample R-squared > OOS_R_threshold for Y0 and Y1 predictions
#' 3. Confirms treatment effect is statistically significant (p < treatment_pval_threshold)
#'
#' @examples
#' \dontrun{
...
SanityCheckSyntheticData <- function(synthetic_data, InSampleR_threshold = 0.01, OOS_R_threshold = 0.01, treatment_pval_threshold = 0.05) {
            ...
            lm_model_Y0 <- lm(Y0 ~ X)
            assert_that(summary(lm_model_Y0)$r.squared > InSampleR_threshold, msg = "R-squared for Y0 is not greater than InSampleR_threshold")
            lm_model_Y1 <- lm(Y1 ~ X)
            assert_that(summary(lm_model_Y1)$r.squared > InSampleR_threshold, msg = "R-squared for Y1 is not greater than InSampleR_threshold")
            lm_model_obsY <- lm(obsY ~ X)
            assert_that(summary(lm_model_obsY)$r.squared > InSampleR_threshold, msg = "R-squared for obsY is not greater than InSampleR_threshold")
            ...
            assert_that(Y0_r2 > OOS_R_threshold, msg = "R-squared for Y0 is not greater than OOS_R_threshold")
            assert_that(Y1_r2 > OOS_R_threshold, msg = "R-squared for Y1 is not greater than OOS_R_threshold")
